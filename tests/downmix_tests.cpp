...
int main()
{
    // --- 1. the fold is the exact arithmetic mean ---
    {
        ...
        const wav::Bytes folded = wav::downmixedToMono(source);
    ...
    // --- 4. no frame moves ---
    {
        ...
        const wav::Info after = wav::readWavInfo(wav::downmixedToMono(source));
    ...
    // --- 5. the output is the pedal's canonical float32 shape ---
    {
        ...
        const wav::Bytes twice = wav::downmixedToMono(folded);
        CHECK(twice == folded);
    }
    ...
    // --- 5b. a DAW chunk before the audio does not shift the fold ---
    {
        ...
        const wav::Bytes folded = wav::downmixedToMono(padded);
        CHECK_EQ(folded.size(), wav::kCanonicalFloatDataStart + 2u * 8u);
    ...
    // --- 6. anything that is not the pedal's own stereo float32 is refused ---
    {
        ...
        CHECK_THROWS(wav::downmixedToMono(truncated), "truncated");
        CHECK_THROWS(wav::downmixedToMono(std::vector<unsigned char> { 'n', 'o' }),
    ...
    // --- isDualMono answers "would folding change this file" ---
    {
        ...
        // agree, or the command could fold the same slot forever.
        {
            ...
            CHECK(wav::isDualMono(wav::downmixedToMono(floatWav(stereo))));
        }
    }
