...
| **SQLite** 3.53.4 | fetched, pinned by the SHA3-256 sqlite.org publishes; the official amalgamation, **compiled into the app** as the history store | Public domain | https://sqlite.org — its authors disclaim copyright, so nothing is owed, and it is listed so nobody has to wonder. One engine on all three platforms rather than whichever `libsqlite3` the system ships. |
